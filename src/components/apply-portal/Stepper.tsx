        ...
        {WIZARD_STEPS.map((item) => {
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center group relative z-10 transition-all ${isFuture ? 'cursor-not-allowed opacity-60' : 'cursor-pointer'
                }`}
            >
              {/* Step Circle */}
              <div
                className={`w-12 h-12 rounded-xl flex items-center justify-center font-bold text-sm transition-all duration-200 ${isDone
              ...
              <div className="mt-2.5 text-center">
                <span
                  className={`block text-[11px] font-bold tracking-wider uppercase ${isCurrent ? 'text-[#0071E3]' : isDone ? 'text-neutral-700' : 'text-neutral-400'
                    }`}
                >
                  Step {item.step}
                </span>
                <span
                  className={`block text-xs font-semibold mt-0.5 ${isCurrent ? 'text-neutral-900' : isDone ? 'text-neutral-700' : 'text-neutral-400'
                    }`}
                >
                  {item.title}
                </span>
