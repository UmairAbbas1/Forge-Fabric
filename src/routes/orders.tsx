            ...
            {(activeCustomerSubmissions.length > 0 || awaitingCustomerApproval.length > 0) && (
              <div className="glass-surface rounded-2xl p-4 sm:p-5 border border-white/80 dark:border-white/[0.08] shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center gap-3.5">
                  <div className="h-10 w-10 rounded-xl bg-[#0071E3]/10 text-[#0071E3] flex items-center justify-center shrink-0">
                    <ClipboardList className="h-5 w-5" />
                  </div>
                  <div>
                    <div className="flex items-center gap-2 flex-wrap">
                      <h2 className="text-base font-bold text-foreground tracking-tight">
                        Order Intake &amp; Technical Review
                      </h2>
                      {awaitingCustomerApproval.length > 0 ? (
                        <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/20">
                          ...
                          <span>{awaitingCustomerApproval.length} Awaiting Your Approval</span>
                        </span>
                      ) : (
                        <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-[#0071E3]/10 text-[#0071E3] border border-[#0071E3]/20">
                          <CheckCircle2 className="h-3 w-3" />
                          <span>{activeCustomerSubmissions.length} Under Merchandiser Review</span>
                        </span>
                      )}
                    </div>
                    <p className="text-xs text-muted-foreground mt-0.5">
                      {awaitingCustomerApproval.length > 0 
                        ? "Review drafts entered by your merchandiser before factory work order release."
                        : "Your order specifications and fabric yields are being verified by our engineering desk."}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2.5 shrink-0">
                  <Link
                    to="/apply/new"
                    className="px-4 py-2 rounded-xl bg-[#0071E3] hover:bg-[#0077ED] text-white font-bold text-xs shadow-md shadow-[#0071E3]/20 flex items-center gap-1.5 transition-all cursor-pointer active:scale-98"
                  >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {awaitingCustomerApproval.map((sub) => (
                    <Link
                      ...
                      className="glass-surface rounded-2xl border border-amber-500/30 p-4 shadow-2xs hover:shadow-xs transition-all flex items-center justify-between gap-3 group cursor-pointer"
                    >
                      <div>
                        <div className="text-[10px] font-bold uppercase tracking-wider text-amber-600 dark:text-amber-400 mb-0.5">
                          Entered by Merchandiser
                        </div>
                        <div className="font-bold text-foreground text-sm font-mono">{sub.apply_reference_code}</div>
                        <div className="text-xs text-muted-foreground mt-0.5">{sub.product_type || sub.submission_type?.replace(/_/g, " ")}</div>
                      </div>
                      <span className="shrink-0 bg-[#0071E3] group-hover:bg-[#0077ED] text-white text-xs font-bold px-3.5 py-1.5 rounded-xl flex items-center gap-1.5 transition-all shadow-xs">
