...
async function inspectSkuMappingsSchema() {
  const { data, error } = await supabase.from("sku_mappings").select("*").limit(1);
  if (error) {
    console.error("Error inspecting sku_mappings:", error.message);
  } else {
    console.log("sku_mappings table is accessible!");
  }
}

inspectSkuMappingsSchema();
